...
# Install the WordPress test suite + Action Scheduler for local PHPUnit runs.
...
install_action_scheduler() {
	local AS_DIR="$WP_CORE_DIR/wp-content/plugins/action-scheduler"
	if [ -d "$AS_DIR" ]; then
		return
	fi
	mkdir -p "$AS_DIR"
	download https://downloads.wordpress.org/plugin/action-scheduler.zip "$TMPDIR/action-scheduler.zip"
	unzip -q -o "$TMPDIR/action-scheduler.zip" -d "$WP_CORE_DIR/wp-content/plugins/"
}
